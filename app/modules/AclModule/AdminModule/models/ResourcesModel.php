...
class ResourcesModel extends BaseModel 
{
    ...
    public function hasChildNodes($parent_id) {
        $sql = dibi::query('SELECT * FROM [' . self::ACL_RESOURCES_TABLE . '] WHERE %and;', array('parent_id' => $parent_id));
        return count($sql);
    }
    ...
    public function getChildNodes($parent_id) {
        $sql = dibi::query('SELECT id, name, comment FROM [' . self::ACL_RESOURCES_TABLE . '] WHERE %and ORDER BY name;', array('parent_id' => $parent_id));
        return $sql->fetchAll();
    }
    ...
    public function getParents($parent_id, &$array, $depth) {
        $sql = dibi::query('SELECT id, name FROM [' . self::ACL_RESOURCES_TABLE . '] WHERE %and ORDER BY name;', array('parent_id' => $parent_id));
        $rows = $sql->fetchAll();
        foreach ($rows as $row) {
