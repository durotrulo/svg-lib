...
abstract class WebLoader extends Control {
	...
	public function render() {
		...
		if ($hasArgs) {
			...
			// if from component, we have already array **ADDED**
			$args = func_get_args();
			if ($hasArgs == 1 and is_array($args[0])) {
	...
	public function getGeneratedFilename(array $files = null) {
		...
		// added to improve readibility **ADDED**
		if ($this->includeFileNames && count($files) == 1) {
			$prefix = $this->generatedFileNamePrefix . substr(basename($files[0]), 0, -3) . '-';
		} else {
			$prefix = $this->generatedFileNamePrefix;
		}
		
//		return $this->generatedFileNamePrefix . md5( **ADDED**
		return $prefix . md5(
