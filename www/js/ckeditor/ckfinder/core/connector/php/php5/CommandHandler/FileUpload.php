...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        /* ADDED */
        // add random 6-characters long suffix to uploaded file name to prevent guessing file names by users - files are public for users
		$token = substr(md5(uniqid()), 0, 6);
		$ext = strtolower(pathinfo($uploadedFile['name'], PATHINFO_EXTENSION));
		$uploadedFile['name'] = preg_replace("/\.$ext$/", "-$token.$ext", $uploadedFile['name']);
        /* ADDED END */
